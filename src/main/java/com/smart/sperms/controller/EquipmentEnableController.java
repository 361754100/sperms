...
public class EquipmentEnableController {
    ...
    public PageSearchWrapper pageSearch(@RequestParam int pageNo, @RequestParam int pageSize, @RequestParam(required = false) Integer e_enable) {
        if(e_enable == null) {
            e_enable = -1;
        }
        PageSearchWrapper wrapper = equipmentEnableService.queryPage(pageNo, pageSize, e_enable);
